...
proc ms:unnuke { nick uhost hand chan arg } {
    ...
    set SQL_INSERT "UPDATE ${::PREEDb0t::mysql_(table)} SET "
    append SQL_INSERT          "${::PREEDb0t::db_(delpre)}='0', ${::PREEDb0t::db_(moddelpre)}='0', ${::PREEDb0t::db_(modnuke)}='0', ${::PREEDb0t::db_(modunnuke)}='0', ${::PREEDb0t::db_(nuke)}='0', ${::PREEDb0t::db_(undelpre)}='0', "
    append SQL_INSERT          "${::PREEDb0t::db_(unnuke)}='1', $db_(unnuke_ctime)='$unnuke_(time)', $db_(unnuke_reason)='$unnuke_(raison)', $db_(unnuke_net)='$unnuke_(net)' "
    append SQL_INSERT       "WHERE ${::PREEDb0t::db_(rlsname)}='$unnuke_(release)'"
    ...
    if { [::mysql::insertid ${::PREEDb0t::mysql_(handle)}] != "" } {
        putlog "La release \002${unnuke_(release)}\002 à été UNNUKE par ${chan}/${nick} (ID: [::mysql::insertid ${::PREEDb0t::mysql_(handle)}])";
